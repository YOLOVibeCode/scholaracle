...
describe('transformSkywardExtract', () => {
  ...
  it('should create course ops with period, time, teacher, room', () => {
    ...
    const mathCourse = courses.find(
      (c) => (c.record as Record<string, unknown>)['title'] === 'AP Mathematics'
    );
  ...
  it('should create grade snapshot with parsed percentage and letter', () => {
    ...
    const mathGrade = grades.find((g) =>
      (g.key as { courseExternalId?: string }).courseExternalId?.includes('ap-mathematics')
    );
    expect(mathGrade).toBeDefined();
    const rec = mathGrade!.record as Record<string, unknown>;
    expect(rec['percentGrade']).toBe(92);
    expect(rec['letterGrade']).toBeUndefined();
  ...
  it('should use content-based IDs for missing assignments (not index-based)', () => {
    ...
    const missing = ops.filter(
      (o) =>
        o.entity === 'assignment' && (o.record as Record<string, unknown>)['status'] === 'missing'
    );
