...
test.describe('@dashboard Layer 2: Parent Dashboard Pages', () => {
  ...
  test('DASH-P-009: Dashboard shows real alerts and deadlines', async ({ page }) => {
    ...
    await page
      .waitForSelector('[data-testid="loading-skeleton-dashboard"]', {
        state: 'hidden',
        timeout: 5000,
      })
      .catch(() => {});
  ...
  test('DASH-P-002: Students List page renders', async ({ page }) => {
    ...
    const hasStudents =
      (await page.locator('[data-testid="student-list"], [data-testid="empty-state"]').count()) > 0;
  ...
  test('DASH-P-003: Add Student page renders', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="input-student-name"], input[name="name"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="input-student-grade"], input[name="grade"]')
    ).toBeVisible();
  });
  ...
  test('DASH-P-005: Alerts page renders', async ({ page }) => {
    ...
    const hasContent =
      (await page.locator('[data-testid="alerts-list"], [data-testid="empty-state"], h1').count()) >
      0;
  ...
  test('DASH-P-007: Notification Settings section renders', async ({ page }) => {
    ...
    const notificationSection = page.locator(
      '[data-testid="notification-settings"], :has-text("Notification"), [role="tab"]:has-text("Notification")'
    );
    const count = await notificationSection.count();

    if (count > 0) {
      await notificationSection.first().click();
      await expect(page.locator('[data-testid="toggle-push"]').first()).toBeVisible({
        timeout: 2000,
      });
    } else {
  ...
  test('DASH-P-008: Grade Thresholds section renders', async ({ page }) => {
    ...
    const thresholdSection = page.locator(
      '[data-testid="threshold-settings"], :has-text("Threshold"), [role="tab"]:has-text("Threshold")'
    );
    const count = await thresholdSection.count();

    if (count > 0) {
      await thresholdSection.first().click();
      await expect(page.locator('[data-testid="input-grade-drop-threshold"]').first()).toBeVisible({
        timeout: 2000,
      });
    } else {
  ...
  test('DASH-P-010: Account page renders', async ({ page }) => {
    await page.goto('/dashboard/account');
    await expect(page).toHaveURL('/dashboard/account');
    await expect(page.locator('[data-testid="dashboard-account-page"]')).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Account' })).toBeVisible();
    await expect(page.getByText(/Manage your account and security/)).toBeVisible();
    await expect(page.getByRole('button', { name: /Send password reset email/i })).toBeVisible();
  });

  test('DASH-P-011: Account page has link to Settings', async ({ page }) => {
    await page.goto('/dashboard/account');
    await expect(page.getByRole('link', { name: /Open Settings/i })).toBeVisible();
    await expect(page.getByRole('link', { name: /Open Settings/i })).toHaveAttribute(
      'href',
      '/dashboard/settings'
    );
  });
