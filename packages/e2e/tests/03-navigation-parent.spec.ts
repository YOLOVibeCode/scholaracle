...
test.describe('@navigation Layer 3: Parent Navigation', () => {
  ...
  test('NAV-P-006: Logo navigates home', async ({ page }) => {
    ...
    const logo = page.locator('[data-testid="logo"], a[href="/dashboard"], img[alt*="logo"]').first();
  ...
  test('NAV-P-014: Mobile nav links work', async ({ page }) => {
    ...
    if (toggleCount > 0) {
      ...
      // Click students link
      const studentsLink = page.locator('[data-testid="mobile-menu"] a[href="/dashboard/students"], nav a:has-text("Students")');
