              ...
              <>
                <Table>
                  ...
                  <TableBody>
                    {caches.map((c) => (
                      <TableRow key={c.id}>
                        <TableCell className="font-medium">{c.platformName}</TableCell>
                        <TableCell className="max-w-[200px] truncate" title={c.loginUrl}>
                          {c.loginUrl}
                        </TableCell>
                        <TableCell className="text-muted-foreground">
                          {c.createdAt ? new Date(c.createdAt).toLocaleString() : '—'}
                        </TableCell>
                        <TableCell>{c.generatedByEmail ?? c.generatedBy ?? '—'}</TableCell>
                        <TableCell>
                          {c.scraperCodeLength + c.transformerCodeLength} chars
                        </TableCell>
                        <TableCell>
                          <div className="flex gap-1">
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => openCacheDetail(c.id)}
                              data-testid={`cache-view-${c.id}`}
                            >
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => purgeCache(c.id)}
                              data-testid={`cache-purge-${c.id}`}
                            >
                ...
                {cacheTotalPages > 1 && (
                  <div className="mt-4 flex justify-between">
                    ...
                    <span className="text-sm text-muted-foreground">
                      Page {cachePage} of {cacheTotalPages}
                    </span>
                    <Button
                      variant="outline"
                      size="sm"
            ...
            {loading === 'reports' ? (
              ...
              <>
                <Table>
                  ...
                  <TableBody>
                    {reports.map((r) => {
                      const count = failureCountByCacheKey[r.cacheKey] ?? 1;
                      ...
                      return (
                        ...
                        >
                          <TableCell>{r.platformName ?? '—'}</TableCell>
                          <TableCell className="max-w-[120px] truncate font-mono text-xs" title={r.cacheKey}>
                            {r.cacheKey}
                          </TableCell>
                          <TableCell>
                            {count >= 3 ? (
                          ...
                          <TableCell className="max-w-[300px] truncate text-sm" title={r.error}>
                            {r.error ?? '—'}
                          </TableCell>
                          <TableCell className="text-muted-foreground">
