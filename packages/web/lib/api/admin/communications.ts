...
export const adminCommunicationsApi = {
  async listLogs(
  ...
  ): Promise<IAdminCommunicationLogsResponse> {
    ...
    return apiClient.get<IAdminCommunicationLogsResponse>(
      `/admin/communications/logs${suffix}`,
      true
    );
  },
  ...
  async sendEmail(payload: {
    ...
    content?: string;
    templateId?: string;
  }): Promise<IAdminSendCommunicationResponse> {
    return apiClient.post<IAdminSendCommunicationResponse>(
  ...
  async listTemplates(): Promise<IAdminCommunicationTemplatesResponse> {
    return apiClient.get<IAdminCommunicationTemplatesResponse>(
  ...
  async updateTemplate(
    id: string,
    payload: { name?: string; subject?: string; content?: string; isActive?: boolean }
    ...
    try {
      if (!stepUpToken) {
        return await apiClient.post<{
          ...
          error?: string;
          code?: string;
        }>('/admin/communications/bulk-send', payload, true);
      }
  ...
  async listBatches(): Promise<IAdminCommunicationBatchesResponse> {
    try {
      return await apiClient.get<IAdminCommunicationBatchesResponse>(
        '/admin/communications/batches',
        true
      );
    } catch (error) {
      const e = error as unknown;
  ...
  async getAnalytics(days: number = 30): Promise<IAdminCommunicationsAnalyticsResponse> {
    ...
    return apiClient.get<IAdminCommunicationsAnalyticsResponse>(
      `/admin/communications/analytics?${qs.toString()}`,
      true
    );
  },
};
