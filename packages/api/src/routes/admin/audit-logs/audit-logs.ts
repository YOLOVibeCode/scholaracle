...
function maskSensitive(value: unknown, depth: number = 0): unknown {
  ...
  for (const [k, v] of Object.entries(obj)) {
    ...
    if (redactedKeys.has(key) || key.includes('token') || key.includes('secret') || key.includes('password') || key.includes('authorization')) {
      out[k] = '[REDACTED]';
      continue;
    }
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const rows = docs
          .map((d: any) => {
            const ts = (d.timestamp instanceof Date ? d.timestamp : new Date(d.timestamp)).toISOString();
            return [
              csvEscape(ts),
