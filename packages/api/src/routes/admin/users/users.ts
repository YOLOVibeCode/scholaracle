...
export function adminUsersRouter(config: IAdminUsersRouterConfig): Router {
  ...
  router.post('/', requireAdminStepUp({ database: config.database, jwtSecret: config.jwtSecret }), async (req: Request, res: Response) => {
    try {
      const authReq = requireSuperAdmin(req, res);
      if (!authReq) return;
      ...
      if (!email || !name || !role || !password) {
        res.status(400).json({ success: false, error: 'email, name, role, password are required' });
        return;
      }
      ...
      const created = await adminUserRepository.create({ email, name, role: role as any, passwordHash });
  ...
  router.put('/:id', requireAdminStepUp({ database: config.database, jwtSecret: config.jwtSecret }), async (req: Request, res: Response) => {
    try {
      const authReq = requireSuperAdmin(req, res);
      if (!authReq) return;
