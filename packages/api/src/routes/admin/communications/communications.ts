    ...
    asyncHandler(async (req: Request, res: Response) => {
      try {
        ...
        if (!criteria || (!criteria.role && (!criteria.emails || criteria.emails.length === 0))) {
          res
            .status(400)
            .json({ success: false, error: 'criteria.role or criteria.emails is required' });
          return;
        }
        ...
        if (!finalContent) {
          res
            .status(400)
            .json({ success: false, error: 'content is required (or choose a template)' });
          return;
        }
    ...
    asyncHandler(async (req: Request, res: Response) => {
      try {
        ...
        if (!name || !channel || !type || !subject || !content) {
          res
            .status(400)
            .json({ success: false, error: 'name, channel, type, subject, content are required' });
          return;
        }
