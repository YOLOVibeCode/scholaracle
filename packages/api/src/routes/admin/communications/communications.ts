...
export function communicationsRouter(config: ICommunicationsRouterConfig): Router {
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (!criteria || (!criteria.role && (!criteria.emails || criteria.emails.length === 0))) {
        throw new ValidationError('criteria.role or criteria.emails is required');
      }
      ...
      if (!finalContent) {
        throw new ValidationError('content is required (or choose a template)');
      }
      ...
      if (criteria.emails && criteria.emails.length > 0) {
        ...
        if (criteria.role !== 'parent' && criteria.role !== 'all') {
          throw new ValidationError('Unsupported role segment for users');
        }

        const docs = await config.database.collection('users').find({}).limit(50).toArray();
        recipients = docs
          .filter((d) => !!d._id && !!d['email'])
          .map((d) => ({ id: d._id.toString(), email: String(d['email']) }));
      }

      if (recipients.length === 0) {
        throw new ValidationError('No recipients matched criteria');
      }
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (!name || !channel || !type || !subject || !content) {
        throw new ValidationError('name, channel, type, subject, content are required');
      }
  ...
  router.put(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const ok = await templateRepository.update(
        id,
        updates as Partial<ICommunicationTemplateData>
      );
      if (!ok) {
        throw new NotFoundError('Template not found');
      }
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      res.status(200).json({
        ...
        data: docs.map((d) => ({
          ...
          sentAt: d['sentAt']
            ? (d['sentAt'] instanceof Date
                ? d['sentAt']
                : new Date(String(d['sentAt']))
              ).toISOString()
            : undefined,
        })),
        total,
