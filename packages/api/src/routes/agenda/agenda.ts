...
function makeOccurrenceKey(seriesDoc: any, occurrenceStart: Date): string {
...
function makeAssignmentOccurrenceKey(doc: any, dueAtIso: string): string {
...
export function agendaRouter(config: IAgendaRouterConfig): Router {
  ...
  router.post('/snooze', authMiddleware(authService), async (req: Request, res: Response) => {
    const userId = (req as unknown as { userId?: string }).userId ?? '';
    if (!userId) {
      res.status(401).json({ success: false, error: 'Unauthorized' });
      return;
    ...
    res.status(200).json({ success: true, data: { snoozedUntil: saved.snoozedUntil.toISOString() } });
  });
